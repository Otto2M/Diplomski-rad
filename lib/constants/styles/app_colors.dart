import 'package:flutter/material.dart';

class AppColors {
  // Light theme colors
  static const Color lightBlue = Color(0xFF0C70B6); // P 106-7 C
  static const Color yellow = Color(0xFFF0D637); // 129 C
  static const Color backgroundLight = Color(0xFFFFFFFF); // P 179-1 C
  static const Color darkBlue = Color(0xFF122C4F); // 2767 C
  static const Color buttonRed = Color(0xFFAA0919); // 7621 CP - Red for buttons
  static const Color errorLightRed = Color(0xFFD32F2F);
  static const Color lightRed = Color(0xFFFFCDCD);
  static const Color transparencyBlue = Color(0xA22F7ED3);

  // Dark theme colors
  static const Color primaryDark = Color(0xFF065186);
  static const Color secondaryDark = Color(0xFFF0D637);
  static const Color backgroundDark =
      Color(0xFF091526); // Custom dark background
  static const Color accentDark = Color(0xFF122C4F);
  static const Color buttonRedDark = Color(0xFF930311);
  static const Color errorDarkRed = Color(0xFFBA1F14);

  // General colors (can be used across themes)
  static const Color white = Color(0xFFFFFFFF);
  static const Color black = Color(0xFF000000);
}
